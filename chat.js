...
const io = new Server({cors: {origin: '*', methods:['GET','POST']}}).listen(server);
...
io.sockets.on('connection', (socket) => {
  ...
  socket.on('sendMessage', async (data) => {
    ...
    const curTime = new Date().toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' })
    ...
    if (await io.of('/').adapter.sockets(new Set(roomName)).size < 2) {
      ...
      if (target)
        io.sockets.to(target).emit('newMessage', {roomName:roomName, message:message, time:curTime})
    }
  ...
  socket.on('quit', (data) => {
    ...
    multi
      ...
      .exec((err, result) => {
        ...
        for (let [err, res] of result) {
          if (err !== null) return io.use((sock, next) => next(err))
          else if (+res !== 1) return io.use((sock, next) => next(new Error('room in sorted set or chat list was not deleted')))
        }
      })
